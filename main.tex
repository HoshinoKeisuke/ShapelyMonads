...
\begin{cassumption}
	...
	This is equivalent to saying that in the double category $\dc{PROF}$,
	the class of weights $\Phi_{\rm{pro}}$ of possibly large coproducts of representables (i.e.\ companions)
...
\begin{cdefinition}
	...
	We say that $F$ is \emph{familial at stage $B$} if the $\b{SET}$-profunctor
	$\scr B(B,F)\colon\scr X\harr\scr A$ is a (possibly large) coproduct of companions.
\end{cdefinition}
\begin{clemma}
	Let $F\colon\scr A\arr\scr B$ be a functor and
	$B\colon\scr X\arr[hook]\scr B$ a fully faithful functor.
	The following are equivalent.
	\begin{enumerate}
		\item %
			$F$ is familial at stage $B$.
		\item %
			For each $X\in\scr X$,
			the functor $F$ is familial at stage $B X$.
		\qedhere %
	\end{enumerate}
\end{clemma}
\begin{cdefinition}
...
\end{definition}
\begin{cdefinition}
	For a category $\scr A$, an \emph{arity} of $\scr A$ is a dense full subcategorry $\scr A_a\arr[hook]\scr A$ of $\scr A$.
\end{cdefinition}
\begin{cdefinition}
