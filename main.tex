...
\begin{definition}[2.1]
	\label{def:2.1}
	A small (symmetric) polycategory $\cl C$ comprises a set $\ob(\cl C)$ of \emph{objects};
	sets $\cl C(\b A;\b B)$ of \emph{morphisms} for each pair of lists $\b A=(A_1,\ldots,A_n)$ and $\b B=(B_1,\ldots,B_m)$ of objects;
	and the following further data:
	\begin{itemize}
		\item %
			\emph{Identity} morphisms $\id_A\in\cl C(A;A)$ for each object.
		\item %
			\emph{Composition} operations giving for each $f\in\cl C(\b A;\b B)$ and $g\in\cl C(\b C;\b D)$ and indices $i,j$ with $B_i=C_j$, a morphism
			\[
				g\pcirc ji f\in\cl C(\b C_{<j},\b A,\b C_{>j};\b B_{<i},\b D,\b B_{>i})\ ,
			\]
			here we use comma to denote concatenation of lists, and write $\b C_{<j}$ for the list $(C_1,\ldots,C_{j-1})$ and so on.
		\item %
			\emph{Exchange} operations giving for each $f\in\cl C(\b A;\b B)$ and permutations $\varphi\in\mathfrak S_n$ (the symmetric group on $n$ letters)
			and $\psi\in\mathfrak S_m$ an element
			\[
				\psi\cdot f\cdot \varphi\in\cl C(\b A_\varphi;\b B_{\psi^{-1}})
			\]
...
\end{definition}
If $\cl C$ is a polycategory, then we think of elements of $\ob(\cl C)$ as wire-labels,
and elements of $\cl C(\b A;\b B)$ as boxes of the form \eqref{2.1}.
...
\begin{definition}
	The axioms for a polycategory $\cl C$ are:
	\begin{itemize}
		\item %
			...
			\]
			for all $f\in\cl C(\b A;\b B)$ and valid indices $i,j$.
		\item %
			...
			\end{equation}
			for all $f\in\cl C(\b A;\b B),g\in\cl C(\b C;\b D)$ and $h\in\cl C(\b E;\b F)$ and all indices $i,j,k,\ell$ with $B_i=C_j$ and $D_k=E_\ell$.
...
\begin{definition}
	Given a functor $F\colon\cl A\arr\cl B$ and objects $A\in\cl A$ and $B\in\cl B$,
...
\end{definition}
\begin{cproposition}
	Let $\cl A$ be a category and $A\in\cl A$ an object.
	The following are equivalent.
	\begin{enumerate}
		\item %
			$A$ is an initial object in $\cl A$.
		\item %
...
\begin{cproposition}
	...
	\begin{enumerate}
		\item %
			$A$ is initial in its connected component of $\cl A$.
		\item %
			$A$ is strongly projective to all morphisms in $\cl A$;
			i.e., for each morphism $f\colon X\to Y$ in $\cl A$, the function
			\[
				\cl A(A,f)\colon \cl A(A,X)\arr \cl A(A,Y)
			\]
			is a bijection.
		\qedhere
	\end{enumerate}
...
\end{cproof}
\begin{cdefinition}
	Let $\cl A$ be a category.
	An object $A\in\cl A$ is \emph{linear} if it satisfies the equivalent conditions of \Cref{prop:linear-object}.
\end{cdefinition}
An operation $t\colon B\to TA$ of a monad $\sf T$ on $\cl A$ corresponds to a family of interpretation functions $\llbracket t \rrbracket\colon\cl A(A,X)\arr\cl A(B,X)$,
...
\begin{cdefinition}
	Let $\sf T$ be a monad on a category $\cl A$. We write $T$ for the underlying endofunctor of $\sf T$.
	There is a fully faithful functor $\overline{\llbracket-\rrbracket}\colon\cl A_{\sf T}\arr {[\cl A^{\sf T},\Set]^\op}$
	...
	For each $B\in\cl A$, we write $\llbracket -\rrbracket^B_{\sf T}\colon (B\downarrow T)^\op\arr {[\cl A^{\sf T},\Set]/{\cl A(B,U^{\sf T}-)}}$
...
\end{cdefinition}
\begin{lemma}
	\mycolor{Let $F\colon\cl A\arr\cl B$ be a functor.}
...
\begin{cdefinition}
	Let $\cl A$ be a category and $X,Y\in\cl A$ objects.
	We say that $Y$ \emph{covers} $X$ if there is
	a morphism $Y\to X$ in $\cl A$.
\end{cdefinition}
\begin{cproposition}
	Let $\cl A$ be a category.
	Any object $X$ is covered by at most one linear object in the following sense:
	for linear objects $Y,Y'\in\cl A$, if there are morphisms $f\colon Y\to X$ and $f'\colon Y'\to X$, then there is an isomorphism $h\colon Y\to Y'$ with $f'h=f$.
\end{cproposition}
\begin{cproof}
	\TODO
\end{cproof}
\begin{definition}
	A functor $F\colon\cl A\arr\cl B$ is \emph{familial at stage $B\in\cl B$} if each operation in $B\downarrow F$ is covered by a linear one;
